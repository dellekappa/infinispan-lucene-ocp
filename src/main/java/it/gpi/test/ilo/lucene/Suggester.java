...
public class Suggester extends Lookup implements Closeable {
    ...
    public Suggester(Directory dir, Analyzer analyzer) throws IOException {
     ...
     */
    public Suggester(Directory dir, Analyzer indexAnalyzer, Analyzer queryAnalyzer, int minPrefixChars,
    ...
    public void build(InputIterator iter) throws IOException {
        if (!(iter instanceof IdentifiableInputIterator)){
            throw new IllegalArgumentException("The input must implements IdentifiableInputIterator in order to be used with this suggester");
        }

        IdentifiableInputIterator it = (IdentifiableInputIterator)iter;
        ...
        synchronized (searcherMgrLock) {
            ...
            try {
                ...
                while ((text = it.next()) != null) {
                    ...
                    add(id, text, iter.contexts(), iter.weight(), payload);
                }
     ...
     *  After adding, updating or deleting a batch of new suggestions,
     *  you must call {@link #refresh} in the end in order to
     *  see the suggestions in {@link #lookup} */
    public void add(BytesRef id, BytesRef text, Set<BytesRef> contexts, long weight, BytesRef payload) throws IOException {
        ensureOpen();
        writer.addDocument(buildDocument(id, text, contexts, weight, payload));
    }

    /** Updates a previous suggestion, matching the exact same
     *  id as before.  Use this to change the text, weight or
     *  payload of an already added suggestion.  If you know
     *  this id is not already present you can use {@link
     *  #add} instead.  After adding, updating or deleting a batch of
     *  new suggestions, you must call {@link #refresh} in the
     *  end in order to see the suggestions in {@link #lookup} */
    public void update(BytesRef id, BytesRef text, Set<BytesRef> contexts, long weight, BytesRef payload) throws IOException {
        ensureOpen();
        writer.updateDocument(new Term(ID_FIELD_NAME, id.utf8ToString()),
                buildDocument(id, text, contexts, weight, payload));
    }

    /** Deletes a previous suggestion, matching the exact same
     *  id as before.  After adding, updating or deleting a batch of
     *  suggestions, you must call {@link #refresh} in the
     *  end in order to see the suggestions in {@link #lookup} */
    public void delete(BytesRef id, BytesRef text, Set<BytesRef> contexts, long weight, BytesRef payload) throws IOException {
        ensureOpen();
        writer.deleteDocuments(new Term(ID_FIELD_NAME, id.utf8ToString()));
    }

    private Document buildDocument(BytesRef id, BytesRef text, Set<BytesRef> contexts, long weight, BytesRef payload) {
        String idString = id.utf8ToString();
        String textString = text.utf8ToString();
        Document doc = new Document();
        FieldType ft = getTextFieldType();

        doc.add(new StringField(ID_FIELD_NAME, idString, Field.Store.NO));
    ...
    public List<LookupResult> lookup(CharSequence key, BooleanQuery contextQuery, int num, boolean allTermsRequired, boolean doHighlight) throws IOException {
        ...
        LiveIndexWriterConfig conf = (writer != null ? writer.getConfig() : getIndexWriterConfig(getGramAnalyzer(), IndexWriterConfig.OpenMode.APPEND));
        final SortingMergePolicy sortingMergePolicy = (SortingMergePolicy) conf.getMergePolicy();
